                            ...
                            appointments?.length > 0 ? appointments?.map((item: any, index: number) => (<View key={item?.id} className='w-[330px] bg-white rounded-lg mx-auto'>
                                ...
                                <View className='border-b-[1px] border-b-[#61CEFF33]'>
                                    <View className="flex-1 gap-y-1 px-6 py-3">
                                        <Text className="text-lg text-[#151B19] font-bold">{item?.doctor?.user?.username}</Text>
                                        <Text className="text-[#333D3A] font-[400] text-[12px]">{item?.doctor?.user?.email}</Text>
                                        ...
                                        {item?.isApproved ? <Text className="text-green-500 font-[400] text-[12px]">Status : Approved</Text> : <Text className="font-[400] text-[12px] text-red-500">Status : Not Approved</Text>}
                                    ...
                                    <View className='flex flex-col gap-y-4 px-6 py-3'>
                                        {
                                            item?.reports?.length > 0 ? item?.reports?.map((item2: any) => {
                                                ...
                                                return <RecordCard key={item2?.id} imgSrc={item2?.docPath} time={item2?.reportDate} doctorName={'you'} userName={item?.patient?.user?.username} title={item2?.title} />
                                            }) : (
                                    ...
                                    <View className='flex flex-col gap-y-4 px-6 py-3'>
                                        {
                                            item?.prescriptions?.length > 0 ? item?.prescriptions?.map((item2: any) => {
                                                ...
                                                return <RecordCard key={item2?.id} imgSrc={item2?.docPath} time={item2?.prescriptionDate} doctorName={'you'} userName={item?.patient?.user?.username} title={item2?.title} />
                                            }) : (
                                                <Text className='text-center text-base font-semibold'>No Data Available</Text>
                                            )
                                        }
