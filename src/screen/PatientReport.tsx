...
import { View, Text, TextInput, Image, FlatList, TouchableOpacity, StatusBar, SafeAreaView, RefreshControl } from 'react-native';
  ...
  return (
    <><StatusBar backgroundColor="#61CEFF33" barStyle="light-content" />
      <SafeAreaView className="flex-1 bg-[#61CEFF33]">
        ...
        } className='w-[330px] bg-white rounded-lg p-4 mb-4 mx-auto'>
          ...
          <View className='flex flex-col gap-y-4'>
            {loading ? (
              ...
              prescriptions?.length > 0 ? prescriptions?.map((item: any) => (
                <RecordCard key={item?.id} imgSrc={item?.docPath} time={item?.prescriptionDate} doctorName={item?.doctor?.user?.username} userName={item?.patient?.user?.username} title={item?.title} />
              )) : (
